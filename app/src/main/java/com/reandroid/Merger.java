...
public class Merger {
    ...
    public File run(ApkBundle bundle, boolean signApk, boolean force) throws Exception {
        ...
        try (ApkModule mergedModule = bundle.mergeModules(!force)) { // I guess force meant force throw ..
            if (mergedModule.hasAndroidManifest()) {
                ...
                // Remove pairip/Google Integrity license check entries from manifest
                removePairipFromManifest(manifest, application);
    ...
    /**
     * Removes pairip (Google Play Integrity / DRM) entries from AndroidManifest:
     *   - <provider> whose android:name contains "pairip.licensecheck.LicenseContentProvider"
     *   - <uses-permission android:name="com.android.vending.CHECK_LICENSE" />
     * Works generically across all apps (not hardcoded to any specific package).
     */
    private void removePairipFromManifest(AndroidManifestBlock manifest, ResXmlElement application) {
        // --- 1. Remove pairip <provider> from <application> ---
        // Use a snapshot list to avoid ConcurrentModificationException
        List<ResXmlElement> providers = application.listElements("provider");
        if (providers != null) {
            for (ResXmlElement provider : new java.util.ArrayList<>(providers)) {
                ResXmlAttribute nameAttr = provider.searchAttributeByResourceId(AndroidManifest.ID_name);
                if (nameAttr != null) {
                    String nameVal = nameAttr.getValueAsString();
                    if (nameVal != null && nameVal.contains("pairip.licensecheck.LicenseContentProvider")) {
                        application.remove(provider);
                        logger.logMessage("Removed pairip provider: " + nameVal);
                    }
                }
            }
        }

        // --- 2. Remove <uses-permission android:name="com.android.vending.CHECK_LICENSE" /> ---
        ResXmlElement manifestElement = manifest.getManifestElement();
        if (manifestElement != null) {
            List<ResXmlElement> permissions = manifestElement.listElements("uses-permission");
            if (permissions != null) {
                for (ResXmlElement perm : new java.util.ArrayList<>(permissions)) {
                    ResXmlAttribute nameAttr = perm.searchAttributeByResourceId(AndroidManifest.ID_name);
                    if (nameAttr != null) {
                        String nameVal = nameAttr.getValueAsString();
                        if ("com.android.vending.CHECK_LICENSE".equals(nameVal)) {
                            manifestElement.remove(perm);
                            logger.logMessage("Removed pairip permission: " + nameVal);
                        }
                    }
                }
            }
